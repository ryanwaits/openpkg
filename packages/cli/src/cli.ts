  ...
  .action(async (entry, options) => {
    try {
      ...
      if (!entryFile) {
        ...
        console.log(
          chalk.gray(`Auto-detected entry point: ${path.relative(targetDir, entryFile)}`),
        );
      } else {
  ...
  .action(async (url, options) => {
    try {
      ...
      try {
        ...
        const requestBody: {
          source: string;
          options?: { followImports?: boolean; maxDepth?: number };
        } = { source: url };
        ...
        if (followItems.includes('imports') || options.maxDepth !== '5') {
          ...
          if (options.maxDepth !== '5') {
            requestBody.options.maxDepth = Number.parseInt(options.maxDepth, 10);
          }
        }
        ...
        if (showItems.includes('imports')) {
          ...
          if (data.parseErrors && data.parseErrors.length > 0) {
            ...
            for (const error of data.parseErrors) {
              if (error.line && error.column) {
                console.warn(
                  chalk.yellow(`  Line ${error.line}, Column ${error.column}: ${error.message}`),
                );
              } else if (error.line) {
          ...
          if (data.imports && data.imports.length > 0) {
            ...
            if (packageImports.length > 0) {
              ...
              console.log(
                `${hasAbsolute ? '├──' : '└──'} ${chalk.green('Package')} (${packageImports.length})`,
              );
        ...
        if (showItems.includes('spec')) {
          ...
          if (data.spec) {
            ...
            if (exportCount > 0) {
              ...
              const functionExports = data.spec.exports.filter((e) => e.kind === 'function').length;
              const classExports = data.spec.exports.filter((e) => e.kind === 'class').length;
              const variableExports = data.spec.exports.filter((e) => e.kind === 'variable').length;
              ...
              if (functionExports > 0)
                console.log(
                  chalk.gray(`  - ${functionExports} function${functionExports > 1 ? 's' : ''}`),
                );
              if (classExports > 0)
                console.log(chalk.gray(`  - ${classExports} class${classExports > 1 ? 'es' : ''}`));
              if (variableExports > 0)
                console.log(
                  chalk.gray(`  - ${variableExports} variable${variableExports > 1 ? 's' : ''}`),
                );
            ...
            if (showItems.includes('summary') && (exportCount > 0 || typeCount > 0)) {
              ...
              if (exportCount > 0) {
                ...
                const exportsByKind: Record<string, Export[]> = {};
              ...
              if (typeCount > 0) {
                ...
                // Group types by kind
                const typesByKind: Record<string, Type[]> = {};
        ...
        console.error(
          chalk.gray('Make sure the Studio server is running on http://localhost:3000'),
        );
        process.exit(1);
      }
    } catch (error) {
